...
public class Demo01DateFormat {
     ...
     *    parse（）方法声明了一个异常ParseException
     *    如果字符串和构造方法的模式不一样，那么程序就会抛出此异常
     *    如何处理：1。throws继续抛出此异常
     *    2.try  catch自己处理
     */
    private static void demo02() throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日 HH时mm分ss秒");
        //2.Date parse(String source)
        Date date = sdf.parse("2019年04月07日 17时20分33秒");
        System.out.println(date);
    }

    /**
     * 使用DateFormat类中的方法format，吧日期格式化为文本
     * 使用步骤：
     * 1.创建SimpleDateFormat对象，构造方法中传递指定的模式
     * 2.调用SimpleDateFormat对象中的方法format，按照构造方法中指定的模式，把Date日期格式化符合模式的字符串(文本)
     */
